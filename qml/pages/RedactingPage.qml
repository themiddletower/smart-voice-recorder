...
Page {
    ...
    property string fileTitle: filePath ? filePath.split('/').pop() : "Выберите аудиофайл"
    ...
    FilePickerPage {
        ...
        onSelectedContentPropertiesChanged: {
            if (selectedContentProperties && selectedContentProperties.filePath) {
                ...
                var fileNameOnly = selectedContentProperties.fileName || selectedPath.split('/').pop();

                page.lastError = "Загрузка...";
                page.filePath = selectedPath;
                page.originalFileName = fileNameOnly;
                ...
                if (meta && meta.annotations && Array.isArray(meta.annotations) && meta.annotations.length > 0) {
                    page.fileAnnotations = meta.annotations;
                    page.voiceLabels = meta.voiceLabels || [];
                    page.lastError = "Метки загружены из файла";
                } else {
                    ...
                    if (backupMeta && backupMeta.annotations && Array.isArray(backupMeta.annotations) && backupMeta.annotations.length > 0) {
                        page.fileAnnotations = backupMeta.annotations;
                        page.voiceLabels = backupMeta.voiceLabels || [];
                        silenceService.saveMetadata(selectedPath, page.fileAnnotations, page.voiceLabels);
                        page.lastError = "Связь восстановлена по хэшу";
                    } else {
                        var analyzed = audioAnalyzer.analyzeFile(selectedPath);
                        if (analyzed && analyzed.length > 0) {
                            page.fileAnnotations = analyzed;
                            page.lastError = "Метки созданы нейросетью";
                        } else {
                            page.fileAnnotations = [];
                            page.lastError = "Анализатор вернул пустой список фрагментов";
                        }
                        page.voiceLabels = [];
                        silenceService.saveMetadata(selectedPath, page.fileAnnotations, page.voiceLabels);
                    }
    ...
    Component {
        ...
        Dialog {
            ...
            Column {
                ...
                DialogHeader { title: "Сохранить как"; acceptText: "Сохранить" }
                TextField {
                    id: nameField
        ...
        Dialog {
            Column {
                ...
                Label {
                    width: parent.width - Theme.horizontalPageMargin * 2
                    anchors.horizontalCenter: parent.horizontalCenter
                    text: "Есть несохранённые изменения. Выйти без сохранения?"
                    wrapMode: Text.Wrap
                    color: Theme.primaryColor
                }
            }
            onAccepted: {
                // Перед выходом разрешаем навигацию, иначе pop() не сработает
    ...
    AppBar {
        ...
        IconButton {
            ...
            onClicked: {
                if (page.isModified) {
                    // Используем animatorPush вместо обычного push
                    pageStack.animatorPush(exitConfirmDialog)
                } else {
                    // Если изменений нет, временно включаем навигацию, чтобы выпустить пользователя
                    page.backNavigation = true
                    pageStack.pop()
                }
            }
        ...
        Row {
            ...
            IconButton {
                ...
                visible: filePath !== ""
                onClicked: {
                    var metadata = { annotations: fileAnnotations, voiceLabels: voiceLabels }
                    pageStack.push(Qt.resolvedUrl("CloudStoragePage.qml"), {
    ...
    Item {
        ...
        Row {
            ...
            IconButton {
                ...
                enabled: filePath !== "" && !playerControllerRedact.isDecoding
                onClicked: {
                    remorse.execute("Вырезание всей тишины", function() {
                        removeSilenceNow()
                    })
                }
            }
